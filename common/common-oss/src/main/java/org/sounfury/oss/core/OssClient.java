package org.sounfury.oss.core;

import cn.hutool.core.io.IoUtil;
import cn.hutool.core.util.IdUtil;
import org.sounfury.core.constant.Constants;
import org.sounfury.core.utils.DateUtils;
import org.sounfury.core.utils.FileUtils;
...
import org.sounfury.oss.enums.PolicyType;
...
public class OssClient {
    ...
    public UploadResult upload(Path filePath, String key, String md5Digest, String contentType) {
        try {
            ...
            // 等待上传完成并获取上传结果
            CompletedFileUpload uploadResult = fileUpload.completionFuture()
                    .join();
            String eTag = uploadResult.response()
                    .eTag();
    ...
    public UploadResult upload(InputStream inputStream, String key, Long length, String contentType) {
        ...
        try {
            ...
            // 等待文件上传操作完成
            CompletedUpload uploadResult = upload.completionFuture()
                    .join();
            String eTag = uploadResult.response()
                    .eTag();
    ...
    public Path fileDownload(String path) {
        // 构建临时文件
        Path tempFilePath = FileUtils.createTempFile()
                .toPath();
    ...
    public long download(String key, OutputStream out) {
        try {
            ...
            // 输出到流中
            try (ResponseInputStream<GetObjectResponse> responseStream = responseFuture.completionFuture()
                    .join()
                    .result()) { // auto-closeable stream
    ...
    public UploadResult uploadSuffix(byte[] data, String suffix) {
        return upload(new ByteArrayInputStream(data), getPath(properties.getPrefix(), suffix),
                Long.valueOf(data.length), FileUtils.getMimeType(suffix));
    }
